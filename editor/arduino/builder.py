...
class PlcProgramParser:
    ...
    def extractInstances(self, program_list: list, debug_vars_list: list, pous_code: str) -> dict:
        ...
        for path, inst in instances.items():
            ...
            for com in body_sec:
                ...
                self._command_list = [{"com": com, "pushEnd": True, "var_pos": 0}]
                act_type = ""
                vars = []
                act_var_pos = 0
                ...
                while len(self._command_list) > 0:
                    ...
                    if command["pushEnd"]:
                        not_end_push_pos = len(inst["actions"]) - 1
                    ...
                    if com_text.startswith("__SET_VAR(") or com_text.startswith("__SET_LOCATED("):
                        ...
                        (act_var_var_pos, act_is_val) = self.extract_var_pos(var_to_set_com, var["type"], inst)
                        if act_var_var_pos < 0:
                            if var["type"] == 'BOOL':
                                bool_acts = self.extract_bool_str_acts(var_to_set_com, act_var_var_pos, inst)
                                for bool_act in bool_acts:
                                    inst["actions"].insert(not_end_push_pos, bool_act)
                            else:
                                self._command_list.append({"com": var_to_set_com, "pushEnd": False, "var_pos": act_var_var_pos})
                        vars = [{"var_pos": act_var_var_pos, "is_val": act_is_val}]
                    elif com_text.startswith("if ("):
                        None
                    elif "_body__" in com_text:
                        act_type = com_text.split('_')[0]
                        var_to_set = re.search("\(.*->(.*)\)", com_text)[1]
                        act_var_pos = inst["vars"][var_to_set +  ".EN"]["number"]
                        
                    if act_type != "":
                        action = {"act_type": act_type, "vars": vars, "var_pos": act_var_pos}
                        if command["pushEnd"]:
                            inst["actions"].append(action)
                        else:
                            inst["actions"].insert(not_end_push_pos, action)
        return instances                        
    ...
    def getBytesFromInst(self, inst: dict) -> bytes:
        ...
        for action in inst["actions"]:
            ...
            action_bytes += struct.pack("<iB", action["var_pos"], len(action["vars"]))
            # TODO arrays, strings
    ...
    def extract_bool_str_acts(self, str_to_parse: str, var_pos, inst: dict) -> list:
        ...
        #act_to_plc_act = {"and_":"AND", "or_": "OR", "not_": "NOT"}
        actions = list()
        ...
        while (len(conds) > 0):
            ...
            if type(cond["cond"]) in (bp.parsers.sqla.SQLAOr, bp.parsers.sqla.SQLAAnd, bp.parsers.sqla.SQLANot):
                actions.insert(0, {"act_type": cond["cond"].logicop.upper(), "vars": [], "var_pos": cond["pos"]})
                action = actions[0]
                for c in cond["cond"].conditions:
                    if type(c) == bp.parsers.sqla.SQLACondition:
                        (vp, is_val) = self.extract_var_pos(acts[int(c.value)], "BOOL", inst)
                        if vp < 0:
                    ...
                    action["vars"].append({"var_pos": vp, "is_val": is_val})
        return actions
    ...
    def isValue(self, str_to_detect: str):
            if str_to_detect.startswith("__BOOL_LITERAL") or str_to_detect.startswith("__time_to_timespec") or\
    ...
    def extract_var_pos(self, com: str, var_type: str, inst: dict) -> tuple:
        if self.isValue(com):
            var_to_set = self.extractValue(com)
            vvar_pos = self._predefined_temp_vars.get(var_to_set)
            is_val = True
            if vvar_pos is None:
                vvar_pos = len(self._predefined_temp_vars.keys())
                self._predefined_temp_vars[var_to_set] = {"number": vvar_pos, "type": var_type}
            else:
