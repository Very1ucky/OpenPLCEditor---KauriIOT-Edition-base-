...
static void RS485_CONNECT_body__(RS485_CONNECT *data__) {
  ...
  if (__GET_VAR(data__->CONNECT)) {
    success = rs485_connect(data__->BAUDRATE, data__->STOPBITS, data__->ENABLE_TERMINAL_RESISTORS);
  }
  // Initialise TEMP variables

  __SET_VAR(data__->,SUCCESS,,success);
...
static void RS485_GET_NEXT_RECEIVED_PACKET_body__(RS485_GET_NEXT_RECEIVED_PACKET *data__) {
  ...
  __SET_VAR(data__->,RECEIVED_PACKET_SIZE,,received_packet_size);
